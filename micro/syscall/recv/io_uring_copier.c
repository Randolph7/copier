...
void *server(void *arg)
{
	...
	while ((msg_read_count < 8) && (total_msg_len > msg_read_len)) {
		msg_read_count++;
		...
		io_uring_prep_recv_copier(sqe, client_sock, buffer, msg_read_len, 0, buffer, NULL);
	...
	for (int i = 0; i < ud; i++) {
		...
		io_uring_cqe_seen(&ring, cqe);
		// end_cycle = __rdtsc();
		// cycles[i] = end_cycle - start_cycle;
	}
	...
	// for(long q = 0; q < msg_read_count; q++){
	// 	printf("time%ld = %lu\n", q, cycles[q]);
	// }
