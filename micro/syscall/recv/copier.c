...
void *server(void *arg)
{
	...
	int threadFd = syscall(602, -1, RECV_COPIER_THREAD_CORE, QUEUE_TYPE_OUT);
	syscall(605, client_sock, threadFd, QUEUE_TYPE_OUT); //bind
	...
	while (total_msg_len > msg_read_len) {
		...
		if (pfds[0].revents & POLLIN) {
			...
			// start_cycle = __rdtsc();
			bytes_read = syscall(NR_RECVFROM_COPYER, client_sock, buffer, buffer, msg_read_len, 0, NULL);
			// end_cycle = __rdtsc();
			clock_gettime(CLOCK_MONOTONIC, &end);
			// cycles[msg_read_count] = end_cycle - start_cycle;
	...
	printf("read %ld K data, read %d for %ld times, avg latency = %lld\n", total_msg_len_copy / 1024, msg_read_len, msg_read_count,
	       nanoseconds / msg_read_count);

	// for(long i = 0; i < msg_read_count; i++){
	// 	printf("time%ld = %lu\n", i, cycles[i]);
	// }

	can_stop = 1;
	syscall(603, client_sock, threadFd, QUEUE_TYPE_OUT);
	close(client_sock);
	close(server_fd);
	return NULL;
}
