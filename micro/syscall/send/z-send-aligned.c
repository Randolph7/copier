...
void *server(void *arg)
{
	...
	for (int i = 0; i < TIME; i++) {
		...
		while (recv(client_sock, buffer, msg_read_len, 0) > 0)
			;
		send_finish = 0;
	}
...
void *client(void *arg)
{
	...
	for (int m = 0; m < TIME; m++) {
		...
		void *msg;
		int result = posix_memalign(&msg, 4096, MSG_LENGTH * 4096);
		if (result != 0) {
		...
		for (int t = 0; t < send_times; t++) {
			msg_len = send(sock, msg + total_msg_len, msg_send_len, MSG_ZEROCOPY);
			if (msg_len < 0) {
