...
void *client(void *arg)
{
	...
	int threadFd = syscall(602, -1, COPIER_THREAD_CORE, QUEUE_TYPE_IN);
	syscall(605, sock, threadFd, QUEUE_TYPE_IN); //bind
	...
	for (int m = 0; m < TIME; m++) {
		...
		for (t = 0; t < send_times; t++) {
			...
			io_uring_prep_send_copier(sqe, sock, msg + total_msg_len, msg_send_len, 0);
			sqe->user_data = t;
	...
	// for(long q = 0; q < msg_send_count; q++){
	// 	printf("time%ld = %lu\n", q, cycles[q]);
	// }

	syscall(603, sock, threadFd, QUEUE_TYPE_IN);
