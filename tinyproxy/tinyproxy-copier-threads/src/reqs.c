 ...
 */
ssize_t safe_write_copier (int fd, const void *buf, size_t count)
{
        ssize_t len;
        size_t bytestosend;
	const char *buffer = buf;
        ...
        while (1) {
                len = LazySend (fd, buffer, bytestosend, MSG_NOSIGNAL, queue_fd);
 ...
 * again.
 */
ssize_t safe_read_copier (int fd, void *buffer, size_t count)
{
        ssize_t len;

        do {
                len = LazyRecv (fd, buffer, count, 0);
...
static int pull_client_data (struct conn_s *connptr, long int length, int iehack)
{
        ...
        do {
                len = safe_read_copier (connptr->client_fd, buffer,
                                 min (MAXBUFFSIZE, (unsigned long int) length));
                if (len <= 0)
                        goto ERROR_EXIT;

                if (!connptr->error_variables) {
                        if (safe_write_copier (connptr->server_fd, buffer, len) < 0)
                                goto ERROR_EXIT;
                }
...
static void relay_connection (struct conn_s *connptr)
{
        ...
        for (;;) {
                ...
                if (fds[1].revents & MYPOLL_READ) {
                        bytes_received =
                            read_buffer_lazy (connptr->server_fd, connptr->sbuffer);
                        if (bytes_received < 0)
                                break;
                        ...
                        if (connptr->content_length.server == 0)
                                break;
                        if(write_buffer_lazy (connptr->client_fd, connptr->sbuffer) < 0) break;
                }
                if ((fds[0].revents & MYPOLL_READ)
                    && (read_buffer_lazy (connptr->client_fd, connptr->cbuffer) < 0
                  || write_buffer_lazy (connptr->server_fd, connptr->cbuffer) < 0)) {
                        break;
                }
        }

        while (buffer_size (connptr->sbuffer) > 0) {
                if (write_buffer_lazy (connptr->client_fd, connptr->sbuffer) < 0)
                        break;
        }
        ...
        while (buffer_size (connptr->cbuffer) > 0) {
                if (write_buffer_lazy (connptr->server_fd, connptr->cbuffer) < 0)
                        break;
        }
