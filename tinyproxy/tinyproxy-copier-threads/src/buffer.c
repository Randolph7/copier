...
extern struct cp_in_queue* cp_in_queue;
static pthread_mutex_t copier_queue_mutex = PTHREAD_MUTEX_INITIALIZER;

void smartInputDescriptorBufferMemcpyAsyncSend(void *dst, void *src, size_t n)
{
        unsigned int write_index;
        struct cp_in_entry* cp_entry;
        
        pthread_mutex_lock(&copier_queue_mutex);
        write_index = cp_in_queue->write_index;
	cp_entry = &cp_in_queue->entries[write_index];
	cp_entry->type = TYPE_SIMPLE_COPY;
	cp_entry->length = n;
	cp_entry->from_va = src;
	cp_entry->to_va = dst;
        cp_entry->status = 1;
	cp_in_queue->write_index = (write_index + 1) % DEFUALT_CP_IN_ENTRY_NUM;
        pthread_mutex_unlock(&copier_queue_mutex);

        /* log_message(LOG_INFO, "index=%d from %lu, to %lu, len %lu", cp_in_queue->write_index, (unsigned long)src, (unsigned long)dst, n); */
}
/*
 ...
 */
static struct bufline_s *makenewlineCopier (unsigned char *data, size_t length)
{
        ...
        smartInputDescriptorBufferMemcpyAsyncSend(newline->string, data, length);
...
int add_to_bufferCopier (struct buffer_s *buffptr, unsigned char *data, size_t length)
{
        struct bufline_s *newline;
         ...
         * Make a new line so we can add it to the buffer.
         */
        if (!(newline = makenewlineCopier (data, length)))
                return -1;
...
ssize_t read_buffer_lazy (int fd, struct buffer_s * buffptr)
{
        ssize_t bytesin;
        unsigned char *buffer;
        ...
        bytesin = LazyRecv (fd, buffer, READ_BUFFER_SIZE, 0);

        if (bytesin > 0) {
                if (add_to_bufferCopier (buffptr, buffer, bytesin) < 0) {
                        log_message (LOG_ERR,
                                     "readbuff: add_to_buffer() error.");
                        bytesin = -1;
                }
...
#  endif
                ...
                default:
                        log_message (LOG_ERR,
                                     "read_buffer_lazy: read() failed on fd %d: %s",
                                     fd, strerror(errno));
                        bytesin = -1;
                        break;
                }
...
ssize_t write_buffer_lazy (int fd, struct buffer_s * buffptr)
{
        ssize_t bytessent;
        struct bufline_s *line;
        ...
        bytessent =
            LazySend (fd, line->string + line->pos, line->length - line->pos,
                  MSG_NOSIGNAL, queue_fd);
...
#  endif
                ...
                case ENOMEM:
                        log_message (LOG_ERR,
                                     "writebufflazy: write() error [NOBUFS/NOMEM] \"%s\" on "
                                     "file descriptor %d", strerror (errno),
                                     fd);
                        return 0;
                default:
                        log_message (LOG_ERR,
                                     "writebufflazy: write() error \"%s\" on file descriptor %d",
                                     strerror (errno), fd);
                        return -1;
                }
        }
