...
#define COPIER_WAIT(skb)                                                                                                                             \
	{                                                                                                                                            \
		if (unlikely(*(skb->descriptor) != COPIER_BLOCK_DONE)) {                                                                             \
			while (*(skb->descriptor) != COPIER_BLOCK_DONE)                                                                              \
				;                                                                                                                    \
		}                                                                                                                                    \
	}
	...
	if (clone_it) {
		...
		tcp_skb_tsorted_save(oskb)
		{
			if (unlikely(skb_cloned(oskb))){
				if (unlikely(*(oskb->descriptor) != COPIER_BLOCK_DONE)) {                                                                             \
					while (*(oskb->descriptor) != COPIER_BLOCK_DONE)                                                                              \
					;                                                                                                                    \
				}
...
static int add_copyin_to_queue(struct cp_in_queue *q, void *to, void __user *from, size_t n, volatile uint8_t *descriptor)
{
	// printk("async %lu bytes, q = %lu\n", n, (unsigned long)q);
	if (add_entry_to_cp_in_queue(q, to, from, n, descriptor))
		return 0;
	printk("cp_in_queue full\n");
	return n;
}

static __always_inline __must_check size_t COPYER_WRAP(copy_from_iter)(struct cp_in_queue *q, void *addr, size_t bytes, struct iov_iter *i,
...
static __always_inline __must_check bool COPYER_WRAP(copy_from_iter_full)(struct cp_in_queue *q, void *addr, size_t bytes, struct iov_iter *i,
									  volatile uint8_t *descriptor)
{
...
static inline int COPYER_WRAP(skb_do_copy_data_nocache)(struct cp_in_queue *q, struct sock *sk, struct sk_buff *skb, struct iov_iter *from, char *to,
							int copy, int offset)
{
	// printk("skb_do_copy_data_nocache skb = %llu\n", (unsigned long)skb);
	skb->descriptor = &(q->global_descriptor[q->descriptor_index]);
	*(skb->descriptor) = 0;
	q->descriptor_index = (q->descriptor_index + 1) % GLOBAL_DESCRIPTOR_SIZE;
...
static inline int COPYER_WRAP(skb_copy_to_page_nocache)(struct cp_in_queue *q, struct sock *sk, struct iov_iter *from, struct sk_buff *skb,
...
static inline int COPYER_WRAP(skb_add_data_nocache)(struct cp_in_queue *q, struct sock *sk, struct sk_buff *skb, struct iov_iter *from, int copy)
{
...
int COPYER_WRAP(tcp_sendmsg_locked)(struct sock *sk, struct msghdr *msg, size_t size)
{
	struct tcp_sock *tp = tcp_sk(sk);
	...
	struct cp_in_queue *q = ((struct copyer_ctx *)(fget(sk->sk_socket->file->queue_fd_in)->private_data))->queue_in;

	struct page **pages;
	unsigned long page_num;
	struct umm_vmap_cache_node *hash_node;
	unsigned long to_page_end, length = size;
	void *user_addr_base = msg->msg_iter.iov->iov_base;
	void *kva;
	bool hash_hit = false;

#ifdef MULTI_USER
	to_page_end = PAGE_SIZE - (unsigned long)user_addr_base % PAGE_SIZE;
	if (length <= to_page_end) {
		page_num = 1;
	} else {
		length -= to_page_end;
		page_num = length / PAGE_SIZE + 1;
		if (length % PAGE_SIZE)
			page_num++;
	}

	q->last_used_base = (unsigned long)user_addr_base;

	hash_for_each_possible (q->ht, hash_node, node, (unsigned long)user_addr_base)
		if (hash_node->uva == (unsigned long)user_addr_base && likely(hash_node->page_num == page_num)) {
			hash_hit = true;
			break;
		}

	if (likely(hash_hit)) {
		q->last_mapped_base_addr = (void *)hash_node->kva;
	} else {
		pages = kmalloc_array(page_num, sizeof(struct page *), GFP_KERNEL);
		get_user_pages_fast((unsigned long)user_addr_base, page_num, FOLL_WRITE, pages);
		q->last_mapped_base_addr = kva = vmap(pages, page_num, VM_MAP, PAGE_KERNEL) + (unsigned long)user_addr_base % (PAGE_SIZE);
		if (unlikely(q->hash_node_index >= COPIER_HASH_NODE_POLL_SIZE)) {
			printk("hash node list full!\n");
		} else {
			hash_node = &q->hash_nodes[q->hash_node_index];
			q->hash_node_index++;
			hash_node->kva = (unsigned long)kva;
			hash_node->uva = (unsigned long)user_addr_base;
			hash_node->page_num = page_num;
			hash_add(q->ht, &hash_node->node, (unsigned long)user_addr_base);
		}
	}
...
int COPYER_WRAP(tcp_sendmsg)(struct sock *sk, struct msghdr *msg, size_t size)
{
	int ret;

	lock_sock(sk);
	ret = tcp_sendmsg_locked_copyer(sk, msg, size);
	release_sock(sk);

	return ret;
}

int COPYER_WRAP(inet_sendmsg)(struct socket *sock, struct msghdr *msg, size_t size)
{
	...
	return tcp_sendmsg_copyer(sk, msg, size);
}
 ...
 *	the protocol.
 */
int COPYER_WRAP(__sys_sendto)(int fd, void __user *buff, size_t len, unsigned int flags, struct sockaddr __user *addr, int addr_len)
{
	struct socket *sock;
	...
	if (sock->file->f_flags & O_NONBLOCK)
		flags |= MSG_DONTWAIT;
	msg.msg_flags = flags;
	err = inet_sendmsg_copyer(sock, &msg, msg_data_left(&msg));
...
SYSCALL_DEFINE6(sendto_copyer, int, fd, void __user *, buff, size_t, len, unsigned int, flags, struct sockaddr __user *, addr, int, addr_len)
{
	return __sys_sendto_copyer(fd, buff, len, flags, addr, addr_len);
}
...
int tcp_sendmsg_locked_copyer_lazy(struct sock *sk, struct msghdr *msg, size_t size, int lazy_fd)
{
	...
	struct cp_in_queue *q = ((struct copyer_ctx *)(fget(lazy_fd)->private_data))->queue_in;
...
int COPYER_WRAP(sock_sendmsg)(struct socket *sock, struct msghdr *msg)
{
	...
	return err ?: inet_sendmsg_copyer(sock, msg, msg_data_left(msg));
}
