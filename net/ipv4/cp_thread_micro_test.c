...
static inline void copier_memcpy_hybrid(struct micro_cp_queue *queue, struct device *dma_dev, struct micro_cp_entry *entry, unsigned int dma_len,
					unsigned int avx_len)
{
	struct dma_async_tx_descriptor *chan_desc, *chan_desc2;
	const enum dma_ctrl_flags flags = DMA_CTRL_ACK | DMA_PREP_INTERRUPT;
	struct dma_chan *chan = entry->chan;
	...
	int page_num;
	dma_cookie_t cookie, cookie2;
	unsigned int half_dma_len = dma_len / 2;
	...
	if (!from_cached) {
		...
		page_from = from_dma_cache_node->pages;
	...
	if (!to_cached) {
		...
		page_to = to_dma_cache_node->pages;
	...
	// if(dma_len > 8192){
	// 	chan_desc2 = dmaengine_prep_dma_memcpy(chan2, dst + half_dma_len, src + half_dma_len, dma_len - half_dma_len, flags);
	// 	if (IS_ERR_OR_NULL(chan_desc2)) {
	// 		printk("chan_desc error chan2\n");
	// 	}
	// 	cookie2 = dmaengine_submit(chan_desc2);
	// 	dma_async_issue_pending(chan2);

	// 	chan_desc = dmaengine_prep_dma_memcpy(chan, dst, src, half_dma_len, flags);
	// 	if (IS_ERR_OR_NULL(chan_desc)) {
	// 		printk("chan_desc error\n");
	// 	}
	...
	// 	dma_async_issue_pending(chan);
	// } else {
		chan_desc = dmaengine_prep_dma_memcpy(chan, dst, src, dma_len, flags);
		if (IS_ERR_OR_NULL(chan_desc)) {
			printk("chan_desc error\n");
		}
		cookie = dmaengine_submit(chan_desc);
	...
	__memcpy_avx_unaligned(page_to_virt(page_to[0]) + dma_len, page_to_virt(page_from[0]) + dma_len, avx_len);

	// if(dma_len > 8192){
	// 	while (dma_async_is_tx_complete(chan2, cookie2, NULL, NULL) != DMA_COMPLETE || dma_async_is_tx_complete(chan, cookie, NULL, NULL) != DMA_COMPLETE)
	// 	;
	// } else {
...
static inline void copier_memcpy_hybrid_no_cache(struct micro_cp_queue *queue, struct device *dma_dev, struct micro_cp_entry *entry, unsigned int dma_len,
					unsigned int avx_len)
{
	struct dma_async_tx_descriptor *chan_desc, *chan_desc2;
	const enum dma_ctrl_flags flags = DMA_CTRL_ACK | DMA_PREP_INTERRUPT;
...
static inline int micro_thread_background_cp(void *ctx_void)
{
	...
	if (ctx->queue_type == ERMS) {
		...
		kernel_fpu_end();
	} else if (dma && ctx->queue_type == COPIER_LARGE) {
		kernel_fpu_begin();
		queue->report_data.start = ktime_get_ns();
		while (!ctx->should_stop) {
			thread_read_index = queue->thread_read_index;
			...
			if (thread_read_index != usr_write_index) {
				...
				switch (entry->type) {
				case MICRO_WORK:
#define DIVIDE_BASE 8
#define DIVIDE_AVX 5
					avx_len = entry->length * DIVIDE_AVX / DIVIDE_BASE;
					dma_len = entry->length - avx_len;
					entry->chan = chans[0];
					entry->chan2 = chans[1];
					copier_memcpy_hybrid(queue, device, entry, dma_len, avx_len);
					break;
				case MICRO_END:
					queue->report_data.end = ktime_get_ns();
					queue->end = true;
		...
		kernel_fpu_end();
	} else if (dma && ctx->queue_type == COPIER_LARGE_NOCACHE) {
		kernel_fpu_begin();
		queue->report_data.start = ktime_get_ns();
		while (!ctx->should_stop) {
			thread_read_index = queue->thread_read_index;
			...
			if (thread_read_index != usr_write_index) {
				...
				case MICRO_WORK:
					...
					entry->chan = chans[0];
					entry->chan2 = chans[1];
					copier_memcpy_hybrid_no_cache(queue, device, entry, dma_len, avx_len);
					break;
				case MICRO_END:
					queue->report_data.end = ktime_get_ns();
					queue->end = true;
