...
static inline void u2u_copy_user_to_user_core(const void *src, const void *dst, const void *base, const size_t n,
					      const volatile uint16_t *descriptors, const int granularity, int8_t lifo)
{
	...
	if (!lifo) {
		while (bytes) {
			...
			copyout_copier((void *)descriptors + (offset_to_base + offset) / granularity * sizeof(uint16_t), &cp_size, sizeof(uint16_t));
			offset += cp_size;
			bytes -= cp_size;
		}
	} else {
		offset = n / granularity * granularity;
		if ((cp_size = n % granularity)) {
			// copyout_copier((void *)(dst + offset), kernel_map_base + offset, cp_size);
			memcpy((void *)(dst + offset), src + offset, cp_size);
			copyout_copier((void *)descriptors + (offset_to_base + offset) / granularity * sizeof(uint16_t), &cp_size, sizeof(uint16_t));
			offset -= granularity;
...
static inline void u2u_copy_user_to_user_core_short(const void *src, const void *dst, const void *base, const uint16_t n,
						    const volatile uint16_t *descriptors, const int granularity)
{
	const long offset_to_base = dst - base;
	// unsigned long page_start = (unsigned long)src & PAGE_MASK;
	// long ret = 0;
	...
	copyout_copier((void *)descriptors + offset_to_base / granularity * sizeof(uint16_t), &n, sizeof(uint16_t));
...
static inline bool u2u_copy_user_to_user(struct u2u_cp_entry *entry)
{
	...
	if (!entry->lifo) {
		...
		if (size > granularity) {
			if (size % granularity) {
				offset = size / granularity * granularity;
				lifo_cp_size = size - offset;
				u2u_copy_user_to_user_core_short(entry->from + offset, entry->to + offset, entry->base, lifo_cp_size,
								 entry->descriptors, granularity);
			} else {
				offset = size - granularity;
				u2u_copy_user_to_user_core_short(entry->from + offset, entry->to + offset, entry->base, granularity,
								 entry->descriptors, granularity);
			}
			entry->size = offset;
		} else {
...
static void u2u_process_sync_entry(struct u2u_sync_entry *sync_entry, struct u2u_cp_queue *cp_queue)
{
	...
	int i, thread_read_index = cp_queue->thread_read_index, write_index = cp_queue->write_index, granularity;
		...
		if (cp_entry->status == STATUS_WAITING) {
			...
			if (ENTRY_OVERLAP_END(cp_entry, sync_start_addr, sync_end_addr, cp_entry_end)) {
				...
				u2u_copy_user_to_user_core(cp_entry->from + (unsigned long)sync_cp_start_addr - (unsigned long)cp_entry->to,
							   sync_cp_start_addr, base, cp_size, cp_entry->descriptors, granularity, cp_entry->lifo);
				cp_entry->size -= cp_size;
				if (unlikely(!cp_entry->size))
					cp_entry->status = STATUS_DONE;
				if (likely(sync_end_addr == cp_entry_end))
					break;
			} else if (ENTRY_OVERLAP_MIDDLE(cp_entry, sync_start_addr, sync_end_addr, cp_entry_end)) {
				sync_cp_start_addr = U2U_ALIGN_DOWN(sync_start_addr - base, granularity) + base;
				cp_size = U2U_ALIGN_UP(size, granularity);
				sync_cp_end_addr = sync_cp_start_addr + cp_size;
				if (unlikely(sync_cp_start_addr < cp_entry->to)) {
					sync_cp_start_addr = cp_entry->to;
					cp_size = sync_cp_end_addr - sync_cp_start_addr;
				}
				if (unlikely(sync_cp_end_addr > cp_entry_end)) {
					sync_cp_end_addr = cp_entry_end;
					cp_size = sync_cp_end_addr - sync_cp_start_addr;
				}
				u2u_copy_user_to_user_core(cp_entry->from + (unsigned long)sync_cp_start_addr - (unsigned long)cp_entry->to,
							   sync_cp_start_addr, base, cp_size, cp_entry->descriptors, granularity, cp_entry->lifo);
				if (unlikely(sync_cp_end_addr == cp_entry_end)) {
					...
					if (!cp_entry->size)
						cp_entry->status = STATUS_DONE;
				} else if (unlikely(sync_cp_start_addr == cp_entry->to)) {
					cp_entry->size -= cp_size;
					if (!cp_entry->size)
						cp_entry->status = STATUS_DONE;
					else {
						cp_entry->from += cp_size;
						cp_entry->to += cp_size;
					}
				} else {
					if (i + 1 < cp_queue->write_index && cp_queue->entries[i + 1].status == STATUS_SPARSE) {
						cp_queue->entries[i + 1].status = STATUS_WAITING;
						cp_queue->entries[i + 1].base = base;
						cp_queue->entries[i + 1].descriptors = cp_entry->descriptors;
						cp_queue->entries[i + 1].granularity = granularity;
						cp_queue->entries[i + 1].from =
							cp_entry->from + (unsigned long)sync_cp_end_addr - (unsigned long)cp_entry->to;
						cp_queue->entries[i + 1].to = sync_cp_end_addr;
						cp_queue->entries[i + 1].size = (unsigned long)cp_entry_end - (unsigned long)sync_cp_end_addr;

						cp_entry->size = cp_entry->size - cp_queue->entries[i + 1].size - cp_size;
					} else {
						printk("split failed!");
					}
				}
				break;
			} else if (ENTRY_OVERLAP_FRONT(cp_entry, sync_start_addr, sync_end_addr)) {
				cp_size = U2U_ALIGN_UP(size, granularity);
				if (unlikely(cp_size > size))
					cp_size = size;
				u2u_copy_user_to_user_core(cp_entry->from, cp_entry->to, cp_entry->base, cp_size, cp_entry->descriptors,
							   cp_entry->granularity, cp_entry->lifo);
				cp_entry->size -= cp_size;
				if (unlikely(!cp_entry->size))
					cp_entry->status = STATUS_DONE;
				else {
