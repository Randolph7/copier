...
static inline int skb_copy_out(struct cp_entry *entry)
{
	...
	while (remainingLength) {
		...
		copyout_copier(entry->to_va + offsetNow, vaddr + entry->from_offset + offsetNow, copyLength);
		offsetNow += copyLength;
		if (entry->descriptor_buffer) {
			offsetToWrite = (unsigned long)entry->to_va - (unsigned long)entry->to_va_base + offsetNow - 1;
			copyout_copier(entry->descriptor_buffer, &offsetToWrite, sizeof(int64_t));
		}
...
{
	unsigned long offset = (unsigned long)src & (PAGE_SIZE - 1);
...
static inline void copyer_copy_out(struct cp_entry *entry)
{
	switch (entry->type) {
...
inline int thread_background_cp(void *ctx_void)
{
	...
	while (!ctx->should_stop) {
		...
		// 	process_sync_command(&sync_queue->entries[sync_queue->thread_read_index], cp_queue);
		...
		// }
		{
			...
			if (thread_read_index != usr_write_index) {
				copyer_copy_out(&cp_queue->entries[thread_read_index]);
