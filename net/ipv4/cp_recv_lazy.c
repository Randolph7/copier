...
static inline int record_cp_entry_lazy(void *to_va, struct page *from_page, size_t page_offset, long len)
{
	struct cp_entry *submit_entry = &lazy_cp_queue.entries[lazy_cp_queue.write_index];
	...
	lazy_cp_queue.write_index = (lazy_cp_queue.write_index + 1) % DEFUALT_CP_ENTRY_NUM;

	return 0;
}

static inline size_t simple_record_iter_lazy(struct page *page, size_t page_offset, size_t bytes, struct iov_iter *i)
{
...
#define iterate_iovec(i, n, base, len, off, __p, STEP)                                                                                               \
	...
	iterate_and_advance(i, bytes, base, len, off, record_cp_entry_lazy(base, page, page_offset + off, len)) return bytes;
}

static int __skb_datagram_iter_lazy(const struct sk_buff *skb, int offset, struct iov_iter *to, int len, bool fault_short)
{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		if ((copy = end - offset) > 0) {
			...
			if (copy > len)
				copy = len;
			n = simple_record_iter_lazy(page, skb_frag_off(frag) + offset - start, copy, to);
			offset += n;
			if (n != copy)
				goto short_copy;
			if (!(len -= copy))
	...
	skb_walk_frags(skb, frag_iter)
	{
		...
		if ((copy = end - offset) > 0) {
			if (copy > len)
				copy = len;
			if (__skb_datagram_iter_lazy(frag_iter, offset - start, to, copy, fault_short))
				goto fault;
			if ((len -= copy) == 0)
				return 0;
			offset += copy;
...
int skb_copy_datagram_iter_lazy(const struct sk_buff *skb, int offset, struct iov_iter *to, int len)
{
	return __skb_datagram_iter_lazy(skb, offset, to, len, false);
}

static inline int skb_copy_datagram_msg_lazy(const struct sk_buff *from, int offset, struct msghdr *msg, int size)
{
	return skb_copy_datagram_iter_lazy(from, offset, &msg->msg_iter, size);
}
				   ...
				   int *cmsg_flags)
{
	...
	found_ok_skb:
		...
		if (!(flags & MSG_TRUNC)) {
			err = skb_copy_datagram_msg_lazy(skb, offset, msg, used);
			if (err) {
				/* Exception. Bailout! */
				if (!copied)
					copied = -EFAULT;
	...
	skip_copy:
		...
		if (!(flags & MSG_PEEK)) {
			...
			release_entry = &lazy_cp_queue.entries[lazy_cp_queue.write_index];
